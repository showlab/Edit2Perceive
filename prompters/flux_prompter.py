from prompters.base_prompter import BasePrompter
...
class FluxPrompter(BasePrompter):
    def __init__(
        ...
        # self.text_encoder_1: CLIPTextModel = None
    ...
    def encode_prompt_using_clip(self, prompt, text_encoder, tokenizer, max_length, device):
        ...
        # Ensure model is on the expected device; if weights still on CPU move them once
        target_device = torch.device(device)
        first_param = next(text_encoder.parameters(), None)
        if first_param is not None and first_param.device != target_device:
            # Move the whole encoder (one-time cost); keeps downstream tensors on GPU
            text_encoder.to(device=target_device)
        input_ids = input_ids.to(device=target_device)
