...
from .multi_res_noise import multi_res_noise_like
...
class BasePipeline(torch.nn.Module):
    ...
    def preprocess_image(self, image, torch_dtype=None, device=None, pattern="B C H W", min_value=-1, max_value=1):
        ...
        if not isinstance(image, torch.Tensor):
            ...
            # already a tensor with shape [B,C,H,W]
            image = image.to(dtype=torch_dtype or self.torch_dtype, device=device or self.device)
        return image
    ...
    def vae_out_to_np(self, vae_output, pattern="B C H W", task="depth"):
        # Transform a torch.Tensor to numpy.ndarray, in range [min_value, max_value]
        # if pattern != "H W C":
        #     vae_output = reduce(vae_output, f"{pattern} -> H W C", reduction="mean")
        # from b c h w to b h w c, swap the channel to the last dimension
        if pattern != "B H W C":
            vae_output = rearrange(vae_output, f"{pattern} -> B H W C")
        if vae_output.shape[0] == 1:
    ...
    def generate_noise(self, shape, seed=None, rand_device="cpu", rand_torch_dtype=torch.float32, device=None, torch_dtype=None, multi_res_noise=False, multi_res_noise_strength=0.9):
        # Initialize Gaussian noise
        generator = None if seed is None else torch.Generator(rand_device).manual_seed(seed)
        if multi_res_noise:
            noise = multi_res_noise_like(
                torch.empty(shape, device=rand_device, dtype=rand_torch_dtype),
                strength=multi_res_noise_strength,
                generator=generator,
                device=rand_device
            )
        else:
